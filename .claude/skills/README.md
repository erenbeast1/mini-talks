...
| `uipm-design` | Logo, corporate identity, banners, icons, social imagery (renamed from upstream `design`) |
...
rm -rf .claude/skills/{ui-ux-pro-max,ui-styling,uipm-design,design-system,brand,slides,banner-design}
...
Upstream's `design` skill is vendored here as `uipm-design`. Claude Code ships a
built-in skill named `design`, and the built-in wins the name, so the upstream
one never loaded. The rename covers the directory, the `name:` field in
`SKILL.md`, and the 50 self-referencing `skills/design/...` paths in its
references and scripts. Re-apply it after any upstream refresh.
