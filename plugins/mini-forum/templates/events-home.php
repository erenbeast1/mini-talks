...
<?php
/* ═══ WHAT THIS MEMBER HAS JOINED ═══
   Opening Mini-Events used to drop everybody straight into the whole
   programme. A member who has joined something wants to see that first — the
   same way the forum leads with their own posts — so it sits above the
   calendar, and only for them. */
if (is_user_logged_in()) :
    $mfe_uid = get_current_user_id();
    $mfe_mine_up = $wpdb->get_results($wpdb->prepare("
        SELECT e.* FROM $te e
        JOIN $tp p ON p.event_id = e.id
        WHERE p.user_id = %d AND p.status = 'joined'
          AND e.status IN ('published','completed')
          AND DATE(e.start_datetime) >= CURDATE()
        ORDER BY e.start_datetime ASC LIMIT 6
    ", $mfe_uid));
    $mfe_mine_past = $wpdb->get_results($wpdb->prepare("
        SELECT e.* FROM $te e
        JOIN $tp p ON p.event_id = e.id
        WHERE p.user_id = %d AND p.status = 'joined'
          AND e.status IN ('published','completed')
    ...
    ", $mfe_uid));
    $mfe_mine = array_merge($mfe_mine_up, $mfe_mine_past);

    /* Each event type keeps the colour it wears everywhere else on this page,
       so a row here and a card below are recognisably the same event. */
    $mfe_type_colour = array('workshop' => 'red', 'meetup' => 'yellow', 'expert_session' => 'blue',
                             'talkspot' => 'red', 'update' => 'green', 'milestone' => 'green');
    $mfe_type_name   = array('workshop' => 'Workshop', 'meetup' => 'Meetup', 'expert_session' => 'Expert Session',
    ...
    foreach ($mfe_mine as $ev) {
        ...
        $colour   = isset($mfe_type_colour[$ev->event_type]) ? $mfe_type_colour[$ev->event_type] : 'yellow';

        $meta = array(isset($mfe_type_name[$ev->event_type]) ? $mfe_type_name[$ev->event_type] : 'Event',
                      date('g:i A', $ts));
        if (!empty($ev->location_name)) $meta[] = $ev->location_name;

        $tag = $ev->status === 'cancelled' ? 'Cancelled'
             : ($upcoming ? 'Coming up' : 'Been');

        $mfe_rows .= mf_block_get('events.mine.row', array(
            'state'  => $upcoming ? 'is-upcoming' : 'is-past',
            'url'    => esc_url(add_query_arg('event', $ev->slug, $eurl)),
...
<div class="mf-container" style="margin-top:24px">
  <?php mf_block('events.mine', array(
    'count' => count($mfe_mine),
    'rows'  => $mfe_rows,
    'empty' => $mfe_rows === '' ? mf_block_get('events.mine.empty') : '',
  )); ?>
</div>
<?php endif; ?>
